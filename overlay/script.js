...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (tournamentBtn && tournamentPanel) {
    ...
    tournamentBtn.addEventListener('click', () => {
      // Check if tournament is active - if so, show control panel instead
...
function initializeTournament(participants) {
  ...
  console.log('About to show tournament control panel...');
  
  // Debug: Force show control panel immediately
  const debugControlPanel = document.getElementById('tournament-control-panel');
  const debugTournamentPanel = document.getElementById('tournament-panel');
  console.log('Control panel element:', debugControlPanel);
  console.log('Tournament panel element:', debugTournamentPanel);
  
  if (debugControlPanel) {
    debugControlPanel.style.display = 'flex';
    console.log('Control panel display set to flex');
  }
  
  if (debugTournamentPanel) {
    debugTournamentPanel.style.display = 'none';
    console.log('Tournament panel hidden');
  }
...
const randomTournamentBtn = document.getElementById('random-tournament-btn');
const prevPhaseBtn = document.getElementById('prev-phase-btn');
const nextPhaseBtn = document.getElementById('next-phase-btn');
...
if (startTournamentBtn) {
  startTournamentBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      // Force show control panel immediately after tournament init
      setTimeout(() => {
        const controlPanel = document.getElementById('tournament-control-panel');
        const tournamentPanel = document.getElementById('tournament-panel');
        ...
        if (controlPanel) {
          ...
          console.log('Force-showing control panel after tournament init');
        }
      }, 200);
...
if (randomTournamentBtn) {
  randomTournamentBtn.addEventListener('click', () => {
    // Random player names
    const randomPlayerNames = [
      'ProGamer', 'SlotMaster', 'LuckyStreamer', 'BigWinner', 'CasinoKing',
      'SpinLord', 'BonusHunter', 'RollMaster', 'JackpotJoe', 'WildCard',
      'MegaSpin', 'GoldenPlayer', 'DiamondUser', 'CrystalGamer', 'PlatinumPro',
      'ElitePlayer', 'ChampionSpin', 'VictorySlots', 'FortuneSeeker', 'StarPlayer'
    ];
    
    // Get random slots from the slot database
    const getRandomSlots = (count) => {
      if (typeof slotDatabase === 'undefined' || slotDatabase.length === 0) {
        // Fallback slot names if database isn't available
        const fallbackSlots = [
        ...
        return fallbackSlots.slice(0, count);
      }
      
      const shuffled = [...slotDatabase].sort(() => 0.5 - Math.random());
      return shuffled.slice(0, count).map(slot => slot.name);
    };
    ...
    const shuffledPlayerNames = [...randomPlayerNames].sort(() => 0.5 - Math.random());
    const randomSlots = getRandomSlots(playerInputs.length);
    
    // Fill inputs with random data
    playerInputs.forEach((input, index) => {
      if (index < shuffledPlayerNames.length) {
        input.value = shuffledPlayerNames[index];
      }
    });
    
    slotInputs.forEach((input, index) => {
      if (index < randomSlots.length) {
        input.value = randomSlots[index];
      }
    });
    
    // Visual feedback
    randomTournamentBtn.textContent = 'Randomized!';
    randomTournamentBtn.style.background = 'linear-gradient(135deg, #00ff7a 0%, #00e1ff 100%)';
    setTimeout(() => {
      randomTournamentBtn.innerHTML = '<span class="middle-icon">🎲</span><span>Random</span>';
      randomTournamentBtn.style.background = '';
    }, 1500);
    
    console.log('Tournament randomized with players and slots');
  });
}
...
window.testShowControlPanel = function() {
  console.log('Testing control panel display...');
  
  // Hide all panels first
  document.querySelectorAll('.middle-panel').forEach(panel => {
  ...
  if (controlPanel) {
    ...
    controlPanel.style.setProperty('z-index', '1000', 'important');
...
// Tournament Control Panel Functions
function showTournamentControlPanel() {
  console.log('showTournamentControlPanel called');
  ...
  // Also hide any other panel types
  const middlePanel = document.getElementById('middle-panel');
  const randomSlotPanel = document.getElementById('random-slot-panel');
  if (middlePanel) middlePanel.style.display = 'none';
  if (randomSlotPanel) randomSlotPanel.style.display = 'none';
  
  // Force show control panel with important styles
  if (controlPanel) {
    controlPanel.style.setProperty('display', 'flex', 'important');
    controlPanel.style.setProperty('visibility', 'visible', 'important');
    controlPanel.style.setProperty('opacity', '1', 'important');
    ...
    console.log('Control panel styles applied');
    console.log('Control panel computed display:', window.getComputedStyle(controlPanel).display);
    
    // Set up the current match display and listeners
    updateCurrentMatchDisplay();
    setupControlPanelListeners();
  } else {
    console.error('Tournament control panel not found in DOM!');
    return;
  }
  
  // Keep tournament button active and set panel visibility flags
  if (tournamentBtn) {
    tournamentBtn.classList.add('active');
  }
  
  // Update global panel visibility flags
  window.tournamentPanelVisible = true;
  window.panelVisible = false;
  window.randomSlotPanelVisible = false;
}
...
function hideTournamentControlPanel() {
  ...
  // Show tournament setup panel again
  if (tournamentPanel) {
    tournamentPanel.style.display = 'flex';
  }
  
  // Keep tournament button active so panel stays visible
  if (tournamentBtn) {
    tournamentBtn.classList.add('active');
  }
}
...
function updateCurrentMatchDisplay() {
  if (!tournamentData.isActive || !tournamentData.phases[tournamentData.currentPhase]) {
    return;
  }
  
  const currentPhase = tournamentData.phases[tournamentData.currentPhase];
  const currentMatch = currentPhase.matches[tournamentData.currentMatchIndex];
  
  if (!currentMatch || currentMatch.winner) {
    // Move to next match or phase
    if (tournamentData.currentMatchIndex < currentPhase.matches.length - 1) {
      tournamentData.currentMatchIndex++;
    } else {
  ...
  if (contestant1Img && contestant1Name) {
    contestant1Img.src = getSlotImage(currentMatch.participant1.slot);
    contestant1Name.textContent = currentMatch.participant1.player;
  }
  
  if (contestant2Img && contestant2Name && currentMatch.participant2) {
    contestant2Img.src = getSlotImage(currentMatch.participant2.slot);
    contestant2Name.textContent = currentMatch.participant2.player;
  }
  
  // Update progress info
  const currentRoundElement = document.getElementById('current-round');
  const matchesRemainingElement = document.getElementById('matches-remaining');
  
  if (currentRoundElement) {
    currentRoundElement.textContent = `Round: ${currentPhase.name}`;
  }
  
  if (matchesRemainingElement) {
    const totalMatches = tournamentData.phases.reduce((sum, phase) => sum + phase.matches.length, 0);
    const completedMatches = tournamentData.phases.slice(0, tournamentData.currentPhase)
      .reduce((sum, phase) => sum + phase.matches.length, 0) + tournamentData.currentMatchIndex;
    matchesRemainingElement.textContent = `Matches Remaining: ${totalMatches - completedMatches}`;
  }
}
...
function setupControlPanelListeners() {
  ...
  if (winnerBtn1) {
    winnerBtn1.onclick = () => declareWinner(1);
  }
  
  if (winnerBtn2) {
    winnerBtn2.onclick = () => declareWinner(2);
  }
  ...
  if (endTournamentBtn) {
    endTournamentBtn.onclick = () => {
      const winnerPayout = document.getElementById('winner-payout').value || 0;
      const runnerUpPayout = document.getElementById('runner-up-payout').value || 0;
      
      endTournament();
      hideTournamentControlPanel();
      
      // Show payout results
      alert(`Tournament Ended!\nWinner Payout: €${winnerPayout}\nRunner-up Payout: €${runnerUpPayout}`);
    };
  }
}

function declareWinner(contestantNumber) {
  if (!tournamentData.isActive || !tournamentData.phases[tournamentData.currentPhase]) {
    return;
  }
  ...
  if (contestantNumber === 1) {
    currentMatch.winner = currentMatch.participant1;
  } else if (contestantNumber === 2 && currentMatch.participant2) {
    currentMatch.winner = currentMatch.participant2;
  }
  
  // Visual feedback
  const winnerBtn = document.getElementById(`winner-btn-${contestantNumber}`);
  if (winnerBtn) {
    winnerBtn.textContent = '🏆 Winner!';
    winnerBtn.style.background = 'linear-gradient(135deg, #ffd700, #ffed4a)';
  ...
  setTimeout(() => {
    ...
    if (winnerBtn) {
      ...
      winnerBtn.style.background = '';
      winnerBtn.style.color = '';
    }
  }, 2000);
}
...
function advanceToNextPhase() {
  const currentPhase = tournamentData.phases[tournamentData.currentPhase];
  const winners = currentPhase.matches
  ...
  if (winners.length > 1) {
    ...
    const nextPhaseIndex = tournamentData.currentPhase + 1;
    if (nextPhaseIndex < tournamentData.phases.length) {
      tournamentData.phases[nextPhaseIndex].matches = createMatches(winners);
      tournamentData.currentPhase = nextPhaseIndex;
      tournamentData.currentMatchIndex = 0;
    }
...
function completeTournament() {
  const finalPhase = tournamentData.phases[tournamentData.phases.length - 1];
  const winner = finalPhase.matches[0]?.winner;
